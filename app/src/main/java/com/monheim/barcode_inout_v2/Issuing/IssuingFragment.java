...
public class IssuingFragment extends Fragment {
    IssuingFunctions issueFunc = new IssuingFunctions();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_issuing, container, false);
...
//        });
        ...
        etSearchOSDate.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (etSearchOSDate.length() == 10) {
                    ArrayAdapter adapter = new ArrayAdapter(getActivity(), R.layout.spinner_item, issueFunc.GetOs(searchDate));
        ...
        etOsOutBarcode.setOnKeyListener((v, keyCode, event) -> {
            if ((event.getAction() == KeyEvent.ACTION_DOWN) &&
                ...
                solomonID = issueFunc.GetSolomonID(barcode, refNbr, uom);
    ...
    private void UpdateDt(){
        if(issueFunc.GetLastQty(searchDate,refNbr,solomonID,uom)) {
            if (issueFunc.UpdateOsItem(qty, uom)){
                DisplayTotCs();
                ListBarcodeTran(lvOSOut);
    ...
    private void ListBarcodeTran(ListView lvVanOut) {
        List<Map<String, String>> dataList;
        dataList = issueFunc.GetOsList(refNbr, searchDate);
        DisplayTotCs();
    ...
    private void DisplayTotCs(){
        issueFunc.GetTotCs(refNbr, searchDate, tvTotCs,tvTotCaseShot);
        issueFunc.GetTotPcs(refNbr, searchDate, tvTotPcs,tvTotPcsShot);
    }
}